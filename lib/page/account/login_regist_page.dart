...
class _LoginPageState extends State<LoginPage> {
  var serverAddress = new TextEditingController(text: "https://www.xeee.vip");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text("登录"),centerTitle: true,
        ),
                ...
                Row(
                  children: [
                    ...
                    ElevatedButton(
                        ...
                        onPressed: () {
                          HttpDio.login_token(
                                  serverAddress.text, email.text, password.text)
                              .then((LoginResponseEntity result) {
                            if (result.status) {
                              SpUtil.putString(StoreKey.ACCESS_TOKEN.name,
                                  result.accessToken!);
                              SpUtil.putString(StoreKey.REFRESH_TOKEN.name,
                                  result.refreshToken!);
                              SpUtil.putString(
                                  StoreKey.BASE_URL.name, serverAddress.text);
                              SpUtil.putString(
                                  StoreKey.TOKEN_TYPE.name, result.tokenType!);
                              HttpDio.initOptions(serverAddress.text, {
                                "Authorization":
                                    "${result.tokenType} ${result.accessToken}"
                              });
                              HttpDio.user_info().then((value) {
                                SpUtil.putObject(
                                    StoreKey.UERINFO.name, value.toJson());
                                widget.loginSuccess();
